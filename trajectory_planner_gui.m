...
%% TRAJECTORY_PLANNER_GUI  7-axis robot trajectory planning & simulation GUI.
%
...
%  FIGURE & MAIN LAYOUT
% ═══════════════════════════════════════════════════════════════════
fig = uifigure('Name', '7-Axis Robot Arm — Trajectory Planner', ...
...
% ═══════════════════════════════════════════════════════════════════
    ...
    function runSimulation()
        ...
        title(ax, sprintf('7-Axis Robotic Arm — %s', ...
...
% ═══════════════════════════════════════════════════════════════════
    ...
    function [q, qd, tv] = generateTrajectory(wpts, method, Ttot, Nseg, pkv)
        % wpts: 7×N joint-space waypoints
        numWPs = size(wpts, 2);
        ...
        elseif startsWith(method, 'cubicpolytraj')
            % Cubic polynomial
            tWPs = linspace(0, Ttot, numWPs);         % waypoint times
            tv   = linspace(0, Ttot, Nseg * numWPs);   % sample times
        ...
        % Ensure tv is a row vector
        if iscolumn(tv), tv = tv'; end
    end

    function name = extractMethodName(method)
        % Strip Chinese label, keep English name

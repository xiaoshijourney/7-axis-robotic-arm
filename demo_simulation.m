...
% Use generalized IK with position constraint
...
show(robot, qTraj(:,1)', 'Visuals', 'on', 'Collisions', 'off', ...
     'PreservePlot', false, 'Parent', ax);
title(ax, '6-Axis Robotic Arm — Pick & Place Simulation', 'FontSize', 14);
view(ax, [45 25]);
axis(ax, 'equal');
xlim(ax, [-0.9 0.9]);
ylim(ax, [-0.9 0.9]);
...
for frame = 1:totalPts
    ...
    title(ax, sprintf('6-Axis Robotic Arm  |  t = %.1f / %.0f s  |  WP %d/%d', ...
...
%% ── 6. Summary ──────────────────────────────────────────────────
fprintf('\n========== Simulation Complete ==========\n');
fprintf('Robot:    6-DOF, reach = %.2f m\n', norm(T_home(1:3,4)));
fprintf('Waypoints: %d\n', numWP);

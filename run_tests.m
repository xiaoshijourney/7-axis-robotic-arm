%% run_tests.m — Validation tests for 7-axis robotic arm
clear; close all;
fprintf('========================================\n');
fprintf('  7-Axis Robotic Arm — Test Suite\n');
fprintf('========================================\n\n');
...
try
    assert(robot.NumBodies == 9);
    expected = {'pedestal','link1','link2','link3','link4','link5','link6','link7','ee'};
...
try
    qBent = [0, -pi/6, pi/4, -pi/3, 0, -pi/6, 0];
    J = geometricJacobian(robot, qBent, 'ee');
    assert(isequal(size(J), [6 7]));
    assert(rank(J) == 6);
    fprintf('PASS (6x7, rank 6)\n');
    pass = pass + 1;
catch ME
    fprintf('FAIL: %s\n', ME.message);
    fail = fail + 1;
...
try
    ...
    fprintf('PASS (all 7 joints valid)\n');
    pass = pass + 1;
catch ME
    fprintf('FAIL: %s\n', ME.message);
    fail = fail + 1;
...
try
    wpts = [home', home'+[0.2;0;0.1;0;0;0;0], home'];
    [qTraj, qdTraj] = trapveltraj(wpts, 50, 'EndTime', 2);
    assert(isequal(size(qTraj), [7 50]));

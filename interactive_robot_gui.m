...
%% INTERACTIVE_ROBOT_GUI  7-axis robot kinematics demo with dual-mode sliders.
%
%  FK Mode : 7 joint-angle sliders → live 3D view + end-effector pose
...
% ── Figure ────────────────────────────────────────────────────────
fig = uifigure('Name', '7-Axis Robot Arm — Interactive Kinematics', ...
...
% ── Right: control stack ──────────────────────────────────────────
% 4 rows: mode-toggle | sliders (FK or IK) | EE pose | status
% Both FK and IK panels occupy row 2; only one visible at a time.
...
ikBtn.ValueChangedFcn = @(src, ~) modeToggle('IK', src.Value);
    ...
    % FK: live label update during drag; robot animates on pause/release
    function onFKDrag(idx, val)
        state.targetCfg(idx) = val;
        fkVals(idx).Text = sprintf('%.1f', rad2deg(val));
        % Restart debounce timer: if user pauses ≥80ms, animate
        stop(debounceTimer);
        start(debounceTimer);
    end

    % FK: on release, animate to final position immediately
    ...
    function onIKSlider()
        ...
        if strcmp(solInfo.Status, 'success')
            ...
            % Animate from old to new config
...
show(robot, state.config, 'Visuals', 'on', 'Collisions', 'off', ...
     'PreservePlot', false, 'Parent', ax);
title(ax, '7-Axis Robotic Arm  —  Forward Kinematics', ...
      'FontSize', 12, 'FontWeight', 'bold');
